...
output_dir = "PTB_XL_NORM_classification"
...
for clf_name, clf in classifiers.items():
    model_path = os.path.join(output_dir, f'NORM_best_model_{clf_name}.pkl')
...
for clf_name, best_model in best_models.items():
    ...
    print(f"NORM _ {clf_name}:\n", conf_matrix)
    plt.figure(figsize=(8, 6))
    sns.heatmap(conf_matrix, annot=True, cmap='Blues', fmt='.2f')
    plt.title(f'NORM Matrix for {clf_name}')
    plt.xlabel('Predicted Labels')
    plt.ylabel('True Labels')
    plt.savefig(os.path.join(output_dir, f'NORM_confusion_matrix_{clf_name}.png'))  # Save confusion matrix plot
    plt.close()
