...
output_dir = "PTB_XL_HYP_classification"
...
for clf_name, clf in classifiers.items():
    model_path = os.path.join(output_dir, f'HYP_best_model_{clf_name}.pkl')
...
for clf_name, best_model in best_models.items():
    ...
    print(f"HYP _ {clf_name}:\n", conf_matrix)
    plt.figure(figsize=(8, 6))
    sns.heatmap(conf_matrix, annot=True, cmap='Blues', fmt='.2f')
    plt.title(f'HYP Matrix for {clf_name}')
    plt.xlabel('Predicted Labels')
    plt.ylabel('True Labels')
    plt.savefig(os.path.join(output_dir, f'HYP_confusion_matrix_{clf_name}.png'))  # Save confusion matrix plot
    plt.close()
