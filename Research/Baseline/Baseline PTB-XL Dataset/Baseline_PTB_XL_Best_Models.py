...
for clf_name, clf in classifiers.items():
    ...
    # Perform grid search if model is not available
    else:
        print(f"Performing Grid Search for {clf_name}...")
        grid_search = GridSearchCV(
            estimator=clf, param_grid=param_grids[clf_name], cv=5, scoring="accuracy"
        )
...
for clf_name, best_model in best_models.items():
    ...
    # Ensure model can predict probabilities
    ...
    # Adjust the size according to your text
    fig, ax = plt.subplots(figsize=(12, 4))
    ax.text(0.5, 0.5, class_report, fontsize=12, va="center", ha="center")
    ax.axis("off")
    plt.savefig(
        os.path.join(
            output_dir, f"PTB_xl multi_class_classification_report_{clf_name}.png"
        )
    )
    plt.close()
    ...
    plt.savefig(
        os.path.join(output_dir, f"confusion_matrix_{clf_name}_percentages.png")
    )
    plt.close()
