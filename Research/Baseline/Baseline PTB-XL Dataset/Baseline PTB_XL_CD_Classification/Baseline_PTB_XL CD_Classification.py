...
output_dir = "PTB_XL_CD_classification"
...
for clf_name, clf in classifiers.items():
    model_path = os.path.join(output_dir, f"CD_best_model_{clf_name}.pkl")
...
for clf_name, best_model in best_models.items():
    ...
    print(f"CD _ {clf_name}:\n", conf_matrix)
    plt.figure(figsize=(8, 6))
    sns.heatmap(conf_matrix, annot=True, cmap="Blues", fmt=".2f")
    plt.title(f"CD Matrix for {clf_name}")
    plt.xlabel("Predicted Labels")
    plt.ylabel("True Labels")
    plt.savefig(
        os.path.join(output_dir, f"CD_confusion_matrix_{clf_name}.png")
